...
const DEMO_SCENARIOS = [
  {
    ...
    queries: [
      ...
      '按区域（region）统计全年总收入和总利润，计算各区域利润率，从高到低排名，找出表现最好和最差的区域。',
      '按季度对比销售趋势：各季度收入、利润走势如何？Q4 旺季效应是否明显？请分品类分析。',
      '做一张以 region 为行、category 为列的收入透视表，并找出利润率最低、最需要改进的品类组合。',
    ],
  },
  {
    id: 'hr_insights',
    icon: '👥',
    title: 'HR 人才数据洞察',
    description: '深入分析员工薪资、绩效与部门分布，识别高潜力人才与薪资倒挂风险。',
    files: ['employees.csv'],
    queries: [
      '请介绍公司整体人才结构：各部门人数分布、平均薪资水平、整体绩效分布情况。',
      '统计各部门薪资的均值、最大值与标准差，哪些部门内部薪资差异最大？',
      '找出高绩效（performance_score ≥ 4.5）员工，分析他们的部门分布与薪资水平，他们是否获得了应有的回报？',
      '识别潜在薪资倒挂：在职超过 3 年，但薪资低于所在部门平均水平的员工有哪些？请列出名单并说明风险。',
    ],
  },
  {
    id: 'order_product',
    icon: '🔗',
    title: '订单与产品关联分析',
    description: '将订单流水与产品目录跨表关联，分析品类收入、退货率与渠道价值。',
    files: ['products.csv', 'orders.csv'],
    queries: [
      '分别查看 products 和 orders 两张表的结构与基本信息，说明如何通过 product_id 关联它们。',
      '将两张表通过 product_id 合并，统计每个产品类别（category）的总销售额、平均折扣率和订单量。',
      '计算每个产品的退货率（status=Returned 订单占比），找出退货率最高的 Top 5 产品，分析可能原因。',
      '按销售渠道（channel）分析业绩：订单总量、总金额、平均订单金额，哪个渠道最有价值？',
    ],
  },
  {
    id: 'nps_survey',
    icon: '📊',
    title: '用户 NPS 满意度分析',
    description: '解析用户调研数据，计算 NPS、找出满意度差距，挖掘产品改进优先级。',
    files: ['survey_nps.csv'],
    queries: [
      '介绍调研基本情况：样本量、受访者国家与角色分布、平均使用时长。',
      '计算整体 NPS（推荐者 score≥9，被动者 7-8，贬损者≤6），并按国家对比 NPS 差异，哪个市场最需要改进？',
      '对比重度用户（Daily/Several/week）与轻度用户（Monthly/Rarely）的 NPS 和满意度差距，找出差距最大的维度。',
      '分析 NPS 低分用户（score≤4）的主要痛点（main_pain_point），结合用户角色给出产品改进优先级建议。',
    ],
  },
];
